...
| Retry diagnostics | Reports retry path and selected candidate in JSON debug output. | Implemented | `src/cli.rs`; `json_debug_reports_low_word_retry_decision`. |
...
| DOM-backed cleanup primitives | Removes dangerous/noise subtrees and unwraps JavaScript pseudo-links using mutable DOM operations. | Implemented | `src/cleaner.rs`; DOM cleanup pipeline tests. |
...
| DOM byline metadata | Extracts author and published date from rel-author, byline, and time elements near article headings. | Implemented | `metadata--dom-author-date`. |
...
| Domain-specific social/comment extractors | Reference includes specialized extractors for Reddit, Hacker News, X/Twitter, Mastodon, YouTube, and others. | Partially implemented | GitHub discussion preservation is covered by `github-pull-request`; Hacker News listing extraction is covered by `domain--hacker-news-listing.html`; Reddit post/comment extraction is covered by `reddit-discussion`; X/Twitter-style status/thread extraction is covered by `domain--microblog-status-thread.html`; `mastodon.social` status/thread extraction is covered by `domain--federated-status-thread.html`; Bluesky, Threads, and LinkedIn representative post fixtures are covered by `domain--bluesky-thread.html`, `domain--threads-post.html`, and `domain--linkedin-post.html`; YouTube watch-page extraction is covered by `domain--video-watch-page.html`. Broader long-tail social/comment domains remain fixture-led follow-up work. |
| Additional site extractors | Covers LeetCode, LWN, AI conversation pages, and representative social post fixtures. | Implemented | New `domain--*.html` reference fixtures. |
...
| Table and footnote Markdown variants | Converts simple data tables and WordPress footnote blocks into stable Markdown. | Implemented | `markdown--simple-table`, `footnotes--wordpress-block`. |
